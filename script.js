...
function checkOpening() {
    ...
    console.log("Current Game String:", currentGameString);
    console.log("Available Openings:", openings);

    for (const line of openings) {
        if (line.trim() === '') continue; // Skip empty lines

        const [sequence, openingName] = line.split(';');

        console.log(`Checking Opening: ${sequence} -> ${openingName}`);

        // Check if the game string starts with the current sequence
        if (currentGameString.startsWith(sequence)) {
            console.log(`Match Found: ${sequence} -> ${openingName}`);
            matchedOpening = openingName;
        } else {
            console.log(`No Match: ${sequence} -> ${openingName}`);
        }
    }

    // Display the matched opening name
    info.textContent = matchedOpening;
    console.log("Matched Opening:", matchedOpening);
}
